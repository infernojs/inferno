...
describe('Components (JSX)', () => {
	...
	it('should render a basic component and remove property if null', () => {
		...
		expect(container.innerHTML).to.equal(innerHTML('<div><div class="basic"><span>The title is </span></div></div>'));
	...
	describe('should render a stateless component with a conditional state item', () => {
		...
		it('Initial render (creation)', () => {
			...
			expect(container.innerHTML).to.equal(innerHTML('<div><h1>Hello folks</h1><button>toggle</button></div>'));
		});
		...
		it('Second render (update with state change) #3', done => {
			...
			requestAnimationFrame(() => {
				expect(container.innerHTML).to.equal(innerHTML('<div><p>Kalle</p><button>toggle</button></div>'));
				done();
			});
		});
	});
	...
	describe('should render a repeating counter component with component children', () => {
		...
		it('should correctly render as values increase', () => {
			...
			expect(container.innerHTML).to.equal(innerHTML('<div><div>0</div><div>0</div><div>0</div></div>'));

			value++;
			render(<Repeater value={value} />, container);
			expect(container.innerHTML).to.equal(innerHTML('<div><div>1</div><div>1</div><div>1</div></div>'));

			value++;
			render(<Repeater value={value} />, container);
			expect(container.innerHTML).to.equal(innerHTML('<div><div>2</div><div>2</div><div>2</div></div>'));
		});
	});
	...
	describe('should render a component with with mapped text nodes', () => {
		...
		it('should correctly render', done => {
			...
			requestAnimationFrame(() => {
				expect(container.innerHTML).to.equal(innerHTML('<div>isok=true<div><span>a</span><span>b</span></div></div>'));
				done();
			});
		});
	});
	...
	describe('should render a component with conditional boolean text nodes', () => {
		...
		it('should correctly render', done => {
			render(<MyComponent98 />, container);
			setTimeout(() => {
				expect(container.innerHTML).to.equal(innerHTML('<div><div><span>a</span></div></div>'));
				done();
			}, 10);
		});
	});
	...
	describe('should render a component with a conditional list that changes upon toggle', () => {
		...
		it('should correctly render', () => {
			...
			expect(container.innerHTML).to.equal(innerHTML('<div><button>Empty</button><ul><li>No cars!</li></ul></div>'));
		});
	...
	describe('should render a component with a list that instantly changes', () => {
		...
		it('should handle update upon click', done => {
			...
			buttons.forEach(button => button.click());
			setTimeout(() => {
				expect(container.innerHTML).to.equal(innerHTML('<div><button>1</button><div>0</div></div>'));
				done();
			}, 10);
		});
	});
	...
	describe('should render a stateless component with context', () => {
		const StatelessComponent3 = ({ value }, { fortyTwo }) => <p>{value}-{fortyTwo || 'ERROR'}</p>;
		...
		it('should correctly render', () => {
			...
			expect(container.innerHTML).to.equal(innerHTML('<div><button>Increase! 0</button><p>0-42</p></div>'));
		});
		...
		it('should handle update upon click', done => {
			...
			buttons.forEach(button => button.click());
			setTimeout(() => {
				expect(container.innerHTML).to.equal(innerHTML('<div><button>Increase! 1</button><p>1-42</p></div>'));
				done();
			}, 10);
		});
	});
	...
	describe('should render a conditional stateless component', () => {
		...
		it('should correctly render', () => {
			render(<First />, container);
			expect(container.innerHTML).to.equal(innerHTML('<div><button>Increase! 0</button><p>0</p></div>'));
		});
		...
		it('should handle update upon click', done => {
			...
			buttons.forEach(button => button.click());
			setTimeout(() => {
				expect(container.innerHTML).to.equal(innerHTML('<div><button>Increase! 1</button><p>1</p></div>'));
				done();
			}, 10);
		});
	});
	...
	describe('handling JSX spread attributes', () => {
		it('should properly handle multiple attributes using spread', () => {
			...
			expect(innerHTML(container.innerHTML)).to.equal(innerHTML('<input class="foo" id="test">'));
		});
	});
	...
	describe('Swapping Component to DOM node', () => {
		it('Should be able to swap statefull component to DOM list when doing setState', () => {
			...
			class FooBar extends Component {
				...
				render() {
					return <div><span>foo1</span><span>foo2</span><span>foo3</span><span>foo4</span></div>;
				}
			}
		...
		it('Should be able to swap stateless component to DOM list when doing setState', () => {
			...
			const FooBar = () => <div><span>foo1</span><span>foo2</span><span>foo3</span><span>foo4</span></div>;
