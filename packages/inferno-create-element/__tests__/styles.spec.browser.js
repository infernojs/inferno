...
describe('CSS style properties', () => {
	...
	it('should pass for jest', () => {
		expect(true).toBeTruthy();
	});

	if (typeof global !== 'undefined' && !global.usingJest) {
		beforeEach(function() {
		...
		preDefined.forEach(arg => {
			[
				...
				it(test.description, () => {
					...
					expect(style(arg.expected)).toContain(container.firstChild.getAttribute('style'));
					render(test.template(), container);
					expect(container.firstChild.nodeType).toBe(1);
					expect(style(arg.expected)).toContain(container.firstChild.getAttribute('style'));
				});
		...
		preDefined.forEach(arg => {
			[
				...
				it(test.description, () => {
					render(test.template(), container);
					expect(container.firstChild.nodeType).toBe(1);
					expect(style(arg.expected)).toContain(container.firstChild.firstChild.getAttribute('style'));
					expect(style(arg.expected)).toContain(container.firstChild.firstChild.getAttribute('style'));
					render(test.template(), container);
					expect(container.firstChild.nodeType).toBe(1);
					expect(style(arg.expected)).toContain(container.firstChild.firstChild.getAttribute('style'));
		...
		it('Should support changing values and properties', () => {
			...
			expect(container.innerHTML).toBe(innerHTML('<div style="float: left;"></div>'));
