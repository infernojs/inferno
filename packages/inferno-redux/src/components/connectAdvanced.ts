...
type SelectorFactory = (dispatch: Dispatch<any>, options: IConnectOptions) => (state: any, props: any) => any;
...
export const connectAdvanced = (selectorFactory: SelectorFactory, {
	...
	const wrapWithConnect = <T extends Function> (WrappedComponent: T): T => {
		...
		const wrappedComponentName: string = (WrappedComponent as any).displayName
