...
describe('Inferno', () => {
	describe('redux', () => {
		...
		it('should set the displayName correctly', () => {
			...
			).displayName).to.eql('Connect(Foo)');
			...
			).displayName).to.eql('Connect(Bar)');
			...
			).displayName).to.eql('Connect(Component)');
		});
		...
		it('should throw an error if the store is not in the props or context', () => {
			...
			expect(() => renderIntoDocument(<Decorated />)).to.throw(/Could not find "store"/);
		});
		...
		it('should return the instance of the wrapped component for use in calling child methods', () => {
			...
			assert.equal(decorated.getWrappedInstance().someInstanceMethod(), someData);
		...
		it('should allow providing a factory function to mapStateToProps', async () => {
			...
			const mapStateFactory = () => {
				...
				return (state, props) => {
					...
					lastProp = props.name;
					lastVal = state.value;
					lastResult = { someObject: { prop: props.name, stateVal: state.value } };
					return lastResult;
				};
			};
		...
		it('should not call update if mergeProps return value has not changed', async () => {
			...
			const Container = connect(() => ({ a: ++mapStateCalls }), null, () => ({ changed: false }))(
				class Container extends Component {
					render() {
						renderCalls++;
						return <Passthrough {...this.props} />;
		...
		it('should allow to clean up child state in parent componentWillUnmount', async () => {
			const reducer = (state = { data: null }, action) => {
				switch (action.type) {
					case 'fetch': return { data: { profile: { name: 'April' } } };
					case 'clean': return { data: null };
		...
		it('should allow custom displayName', () => {
			const MyComponent = connect(null, null, null, { getDisplayName: name => `Custom(${name})` })(
