...
describe('Router (jsx)', () => {
			...
			render(
				<Router url={'/foo/bar'} history={browserHistoryWithBaseName}>
					<Route path={'/foo'} component={({ children }) => <div><p>Parent Component</p>{children}</div>}>
						<Route
							path={'/:test'}
							component={({ params }) => <div>Child is {params.test} Link is <Link to="/foo/test" /></div>}
						/>
					</Route>
				</Router>,
				container,
	...
	describe('#history', () => {
		...
		it('should render the TestComponent with given paths', () => {
			...
			render(createRouterWithSingleRoute('/foo', '/foo', TestComponent), container);
			expect(container.innerHTML).toBe(innerHTML('<div>Test!</div>'));

			render(createRouterWithSingleRoute('/foo/bar/yar', '*', TestComponent), container);
			expect(container.innerHTML).toBe(innerHTML('<div>Test!</div>'));

			render(createRouterWithSingleRoute('/foo/bar', '/foo/*', TestComponent), container);
			expect(container.innerHTML).toBe(innerHTML('<div>Test!</div>'));
		});
		it('should render the TestComponent with given paths (and params)', () => {
			render(createRouterWithSingleRoute('/foo', '/:test', TestComponentParams), container);
			expect(container.innerHTML).toBe(innerHTML('<div>Test! foo</div>'));

			render(createRouterWithSingleRoute('/foo/bar', '/foo/:test', TestComponentParams), container);
			expect(container.innerHTML).toBe(innerHTML('<div>Test! bar</div>'));

			render(createRouterWithSingleRoute('/foo/bar/yar', '/foo/bar/:test', TestComponentParams), container);
		...
		it('should fail on empty routes', () => {
			expect(() => render(<Router url={'/foo/bar'} history={browserHistory} />, container)).toThrowError(TypeError);
		});
	});
	describe('#RouterContext', () => {
		it('should fail when `location` is not provided', () => {
			expect(() => render(<RouterContext location={null} />, container)).toThrowError(TypeError);
		});
		it('should fail when `matched` is not provided', () => {
			expect(() => render(<RouterContext location={'/'} matched={null} />, container)).toThrowError(TypeError);
		});
