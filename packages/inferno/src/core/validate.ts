...
function getTagName(vNode) {
  ...
  if (flags & VNodeFlags.Element) {
    ...
    const componentName = vNode.type.name || vNode.type.displayName || vNode.type.constructor.name;
  ...
  for (let i = 0, len = vNodeTree.length; i < len; i++) {
    ...
    if (!isInvalid(children)) {
      ...
      if (isArray(children)) {
        ...
        val = DEV_ValidateKeys([children], childNode, forceKeyed || childNode.flags & VNodeFlags.HasKeyedChildren);
      }
      if (val) {
...
export function validateKeys(vNode, forceKeyed) {
  if (process.env.NODE_ENV !== "production") {
    ...
    if (vNode.props && vNode.props.children && vNode.flags & VNodeFlags.Component) {
