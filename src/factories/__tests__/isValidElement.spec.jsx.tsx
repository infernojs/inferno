...
describe('isValidElement', () => {
	...
	it('Should not work with non-object', () => {
		expect(isValidElement(33 as VElement)).to.equal(false);
		expect(isValidElement(false as VElement)).to.equal(false);
		expect(isValidElement(true as VElement)).to.equal(false);
		expect(isValidElement('some text' as VElement)).to.equal(false);
		expect(isValidElement(0 as VElement)).to.equal(false);
		expect(isValidElement(undefined)).to.equal(false);
	});

	it('Should not work with invalid object', () => {
		expect(isValidElement(null as VElement)).to.equal(false, 'object should not be null');
		expect(isValidElement({} as VElement)).to.equal(false, 'object should not be empty');
		expect(isValidElement({ dom: 'fake data' } as VElement)).to.equal(false, 'object should not have just a dom property');
	});
	...
	it('Should work with createElement (element)', () => {
		...
		expect(isValidElement(el as VElement)).to.equal(true);
	});
	...
	it('Should work with createElement (stateless component)', () => {
		...
		expect(isValidElement(comp as VElement)).to.equal(true);
	});
	...
	it('Should not work with a stateless component (using createElement)', () => {
		const el = createElement('div', null, 'Do a thing');
		const Comp = () => el;
		expect(isValidElement(Comp as VElement)).to.equal(false);
	});
