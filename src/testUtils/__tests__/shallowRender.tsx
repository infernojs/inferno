...
import { Inferno013, Inferno15 } from '../src/version';
import { ShallowWrapper, render, shallow } from '../shallowRender';
...
describe('shallow', () => {
  describe('context', () => {
    it('can pass in context', () => {
      const SimpleComponent = Inferno.createClass({
        contextTypes: {
    ...
    describeIf(!Inferno013, 'stateless function components', () => {
      it('can pass in context', () => {
        const SimpleComponent = (props, context) => (
          <div>{context.name}</div>
        );
        SimpleComponent.contextTypes = { name: Inferno.PropTypes.string };
  ...
  describe('.contains(node)', () => {
    ...
    it('should throw on invalid argument', () => {
      ...
      expect(() => wrapper.contains({})).to.throw(
        Error,
        'ShallowWrapper::contains() can only be called with InfernoElement (or array of them), string or number as argument.', // eslint-disable-line max-len
      );
      expect(() => wrapper.contains(() => ({}))).to.throw(
        Error,
        'ShallowWrapper::contains() can only be called with InfernoElement (or array of them), string or number as argument.', // eslint-disable-line max-len
  ...
  describe('.equals(node)', () => {
    ...
    describeIf(!Inferno013, 'stateless components', () => {
  ...
  describe('.find(selector)', () => {
    ...
    it('should find component based on a Inferno prop', () => {
      const wrapper = shallow(
        <div>
          <span title="foo" />
        </div>,
    ...
    it('should find components with multiple matching Inferno props', () => {
      function noop() {}
      const wrapper = shallow(
        <div>
  ...
  describe('.setProps(newProps)', () => {
    ...
    it('should pass in old context', () => {
      ...
      Foo.contextTypes = { x: Inferno.PropTypes.string };
  ...
  describe('.setContext(newContext)', () => {
    ...
    describeIf(!Inferno013, 'stateless functional components', () => {
      const SFC = (props, context) => (
        <div>{context.name}</div>
      );
      SFC.contextTypes = { name: Inferno.PropTypes.string };
  ...
  describe('.simulate(eventName, data)', () => {
    ...
    describe('Normalizing JS event names', () => {
      it('should convert lowercase events to Inferno camelcase', () => {
        const spy = sinon.spy();
      ...
      describeIf(!Inferno013, 'normalizing mouseenter', () => {
        it('should convert lowercase events to Inferno camelcase', () => {
          const spy = sinon.spy();
          class Foo extends Component {
            render() {
        ...
        it('should convert lowercase events to Inferno camelcase in stateless components', () => {
          const spy = sinon.spy();
          const Foo = () => (
            <a onMouseEnter={spy}>foo</a>
          );
  ...
  describe('.isEmptyRender()', () => {
    ...
    itWithData(emptyRenderValues, 'when a Inferno class component returns: ', (data) => {
    ...
    describeIf(Inferno15, 'stateless function components', () => {
    ...
    describeIf(!Inferno013, 'stateless function components', () => {
      ...
      describe('context', () => {
        it('can pass in context', () => {
          ...
          Bar.contextTypes = { name: Inferno.PropTypes.string };
          const Foo = () => (
            <div>
              <Bar />
            </div>
      ...
      it('should provoke an another render to call setState twice in componentDidUpdate', () => {
        ...
        class Foo extends Component {
          ...
          componentDidUpdate() {
            if (!this.updated) {
              this.updated = true;
              /* eslint-disable Inferno/no-did-update-set-state */
              this.setState({ count: this.state.count + 1 });
              this.setState({ count: this.state.count + 1 });
              /* eslint-enable Inferno/no-did-update-set-state */
            }
          }
          render() {
            spy();
  ...
  itIf(Inferno15, 'works with SFCs that return null', () => {
    const Foo = () => null;
    ...
    describe('node with displayName', () => {
      ...
      describe('Inferno.createClass', () => {
        it('should return the name of the node', () => {
          ...
          const Wrapper = Inferno.createClass({
            render() {
              return <Foo />;
            },
          });
